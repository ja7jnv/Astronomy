"""
月の位相をMatplotlibとPyEphemを用いて可視化するためのユーティリティ。
このモジュールは、観測者と月オブジェクトで初期化されるクラス `MoonPhase` を実装。
`draw()` メソッドによって、太陽からの離角に基づいて月の位相を可視化する。
可視化は3Dプロットで行われ、月の照らされた部分は黄色、それ以外は灰色で表示される。
プロットはインタラクティブに更新され、時間とともに月の位相の変化を反映する。
"""
import logging
logging.basicConfig(
    level=logging.DEBUG,
